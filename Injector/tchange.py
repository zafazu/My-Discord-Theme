...
def extract_settings_from_text(text: str):
    ...
    if isinstance(j, dict):
        if "DV_CHANGE" in j and isinstance(j["DV_CHANGE"], dict):        #UNUSED
            return j["DV_CHANGE"]
        if "DV_SETTINGS" in j and isinstance(j["DV_SETTINGS"], dict):
            return j["DV_SETTINGS"]
        return j
...
register_handler("DV_SETTINGS.windowTransparency", handler_window_transparency)
